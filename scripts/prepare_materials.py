...
from consensus_audit.source_materials import split_text, validate_bundle
...
def build_bundle(cache: Path, *, spec_root: Path = ROOT / "audit-specs",
                 dissertation_version: str = REVISION) -> dict:
    inventory = yaml.safe_load((spec_root / "sources.yaml").read_text(encoding="utf-8"))
    sources = []
    blocks = []
    unknowns = [
        "Human confirmation of target path and target revision is pending; no target has been selected.",
        "Human review of PDF conversion and protocol/extension interpretations is pending. Figures other than Figure 2 may lose graphical relationships in text; consult the cached PDFs.",
        "Figure 2 lastApplied persistence must be reconciled with the author's dissertation erratum for a persistent state machine; do not silently choose an interpretation.",
        "PreVote and leader-transfer variants in dissertation sections 4.2.3 and 9.6 require explicit applicability review; fixed membership remains the configured scope.",
        "Asynchronous storage completion and publication contracts require target-version design/API evidence during location; the paper alone does not specify that API.",
        "Read-only lease modes may require clock/availability assumptions absent from the configured fault model. Scope remains included; applicability and assumptions need review.",
    ]
    for original in inventory["sources"]:
        ...
        if sid == "raft-extended":
            ...
            path = spec_root.parent / source["local"]
        if not path.is_file():
...
def main() -> None:
    ...
    parser.add_argument("--cache-dir", type=Path, default=ROOT / ".cache/materials")
    ...
    bundle = build_bundle(args.cache_dir, dissertation_version=args.dissertation_version)
    destination = args.output or args.cache_dir / "source-bundle.json"
